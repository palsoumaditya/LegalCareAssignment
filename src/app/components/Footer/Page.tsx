...
const FooterSecond = () => {
  ...
  return (
    <footer className="bg-black text-white py-8 px-6 shadow-lg">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 mb-8">
          <div className="col-span-1 sm:col-span-2 lg:col-span-1">
            <div className="flex items-center mb-4">
              {brandDetails.brandIcon}
              <span className="text-xl font-bold ml-2 text-white">{brandDetails.brandName}</span>
            </div>
            <p className="text-sm mb-4 text-gray-300" dangerouslySetInnerHTML={{ __html: brandDetails.address }} />
          ...
          <div className="sm:col-span-2 lg:col-span-4 grid grid-cols-2 sm:grid-cols-4 gap-8 ">
            {footerSections.map((section, index) => (
              <div key={index} className="space-y-4">
                <h3 className="font-semibold mb-2 text-sm uppercase tracking-wider text-white">{section.title}</h3>
                <ul className="space-y-2">
                  {section.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      <Link href={link.url} className="text-sm text-gray-300 hover:text-white transition-colors">
                        {link.text}
                      </Link>
                    </li>
                  ))}
